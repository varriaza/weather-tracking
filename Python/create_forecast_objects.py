...
def create_query(json_data: Dict[str, Any], query_type: str) -> Query:
    if query_type not in ["hourly", "daily"]:
        raise ValueError(
            f"Query type must be either 'hourly' or 'daily'. Found {query_type}"
        )
...
def create_forecast_time(
    json_data: Dict[str, Any], num_period: int, query_id: uuid4
) -> ForecastTime:
...
def create_dewpoint(
    ...
    dewpoint_object = Dewpoint(
        dewpoint_id=uuid4(),
        query_id=query_id,
        dewpoint_value=json_data["properties"]["periods"][num_period]["dewpoint"][
            "value"
        ],
        dewpoint_units=json_data["properties"]["periods"][num_period]["dewpoint"][
            "unitCode"
        ],
    )
...
def create_humidity(
    ...
    humidity_object = Humidity(
        humidity_id=uuid4(),
        query_id=query_id,
        humidity_value=json_data["properties"]["periods"][num_period][
            "relativeHumidity"
        ]["value"],
        humidity_units=json_data["properties"]["periods"][num_period][
            "relativeHumidity"
        ]["unitCode"],
    )
...
def create_precipitation(
    ...
    precipitation_object = Precipitation(
        ...
        query_id=query_id,
        probability_of_precip_value=json_data["properties"]["periods"][num_period][
            "probabilityOfPrecipitation"
        ]["value"],
        probability_of_precip_units=json_data["properties"]["periods"][num_period][
            "probabilityOfPrecipitation"
        ]["unitCode"],
    )
...
def create_temperature(
    ...
    temperature_object = Temperature(
        ...
        temperature_units=json_data["properties"]["periods"][num_period][
            "temperatureUnit"
        ],
        temperature_trend=json_data["properties"]["periods"][num_period][
            "temperatureTrend"
        ],
    )
...
def create_wind(json_data: Dict[str, Any], num_period: int, query_id: uuid4) -> Wind:
    ...
    if len(wind_data_list) != 2:
        raise ValueError(
            f"Invalid wind speed format. Expected 2 parts: speed and units. Got {wind_data}"
        )
